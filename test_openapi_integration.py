...
class TestOpenAPIIntegration:
    ...
    async def test_openapi_tools_generated(self, mock_settings):
        ...
        with patch('httpx.AsyncClient') as mock_client_class:
            ...
            openapi_tools_found = [name for name in tool_names if name in expected_openapi_tools]
            assert len(openapi_tools_found) > 0, f"No OpenAPI tools found. Available tools: {tool_names}"
    ...
    async def test_server_initialization_with_auth(self, mock_settings):
        ...
        with patch.dict(os.environ, {
            ...
            with patch('httpx.AsyncClient') as mock_client_class:
                ...
                assert server.settings.auth_provider == 'fastmcp.server.auth.providers.jwt.JWTVerifier'
    ...
    async def test_error_handling_in_tools(self, mock_settings):
        ...
        with patch('httpx.AsyncClient') as mock_client_class:
            ...
            with patch.object(server.metrics, 'to_dict', side_effect=Exception("Test error")):
                result = await metrics_tool.fn()
                result_data = json.loads(result)
