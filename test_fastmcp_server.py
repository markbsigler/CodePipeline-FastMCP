...
class TestValidationFunctions:
    ...
    def test_validate_srid_edge_cases(self):
        ...
        with pytest.raises(
            ValueError, match="SRID must be 1-8 alphanumeric characters"
        ):
    ...
    def test_validate_assignment_id_edge_cases(self):
        ...
        assert (
            validate_assignment_id("12345678901234567890") == "12345678901234567890"
        )  # Max length
        assert validate_assignment_id("ABC123XYZ") == "ABC123XYZ"

        # Test invalid cases
        with pytest.raises(
            ValueError, match="Assignment ID is required and must be a string"
        ):
            validate_assignment_id("")

        with pytest.raises(
            ValueError, match="Assignment ID must be 1-20 alphanumeric characters"
        ):
