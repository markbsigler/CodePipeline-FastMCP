...
class TestMainPy100PercentCoverage:
    """Tests to achieve 100% coverage for main.py"""
    
    def test_bmc_api_rate_limit_error_initialization(self):
        """Test BMCAPIRateLimitError initialization with retry_after."""
        error = main.BMCAPIRateLimitError(
    ...
    def test_bmc_api_validation_error_initialization(self):
        """Test BMCAPIValidationError initialization with validation_errors."""
        error = main.BMCAPIValidationError(
            message="Validation failed",
            validation_errors=["Invalid SRID", "Missing assignment_id"],
            status_code=422,
            response_data={"error": "validation"}
        )
        assert error.message == "Validation failed"
        assert error.validation_errors == ["Invalid SRID", "Missing assignment_id"]
        assert error.status_code == 422
        assert error.response_data == {"error": "validation"}

    def test_bmc_api_validation_error_with_none_validation_errors(self):
    ...
    def test_health_checker_with_psutil_available(self):
        """Test HealthChecker when psutil is available."""
        with patch('main.psutil') as mock_psutil:
            ...
            assert result['system_metrics']['memory_percent'] == 75.0
            assert result['system_metrics']['disk_percent'] == 60.0

    def test_health_checker_with_psutil_import_error(self):
        """Test HealthChecker when psutil import fails."""
    ...
    def test_health_checker_with_psutil_exception(self):
        """Test HealthChecker when psutil raises an exception."""
        with patch('main.psutil') as mock_psutil:
    ...
    def test_bmc_client_make_request_with_retry_after(self):
        """Test BMCAMIDevXClient.make_request with retry_after header."""
        mock_response = Mock()
        mock_response.status_code = 429
        mock_response.headers = {'Retry-After': '60'}
        mock_response.json.return_value = {"error": "rate_limit"}
        
        with patch.object(main.httpx.AsyncClient, 'get', return_value=mock_response):
            client = main.BMCAMIDevXClient(httpx.AsyncClient(), main.IntelligentCache(), main.Metrics(), main.ErrorHandler(main.Settings(), main.Metrics()))
            
            with pytest.raises(main.BMCAPIRateLimitError) as exc_info:
                asyncio.run(client.make_request("GET", "/test"))
            
            assert exc_info.value.retry_after == 60

    def test_bmc_client_make_request_with_validation_errors(self):
        """Test BMCAMIDevXClient.make_request with validation errors."""
        mock_response = Mock()
    ...
    def test_bmc_client_make_request_with_json_parse_error(self):
        """Test BMCAMIDevXClient.make_request with JSON parse error."""
        mock_response = Mock()
    ...
    def test_bmc_client_make_request_with_general_exception(self):
        """Test BMCAMIDevXClient.make_request with general exception."""
        with patch.object(main.httpx.AsyncClient, 'get', side_effect=Exception("Network error")):
            client = main.BMCAMIDevXClient(httpx.AsyncClient(), main.IntelligentCache(), main.Metrics(), main.ErrorHandler(main.Settings(), main.Metrics()))
    ...
    def test_bmc_client_get_cached_or_fetch_with_cache_hit_and_metrics(self):
        """Test BMCAMIDevXClient.get_cached_or_fetch with cache hit and metrics."""
        cache = main.IntelligentCache()
        metrics = main.Metrics()
        client = main.BMCAMIDevXClient(httpx.AsyncClient(), cache, metrics, main.ErrorHandler(main.Settings(), metrics))
        ...
        result = asyncio.run(client.get_cached_or_fetch("test_method", "test_key", fetch_func, param="value"))
        assert result == "cached_data"
        assert metrics.cache_hits == 1

    def test_bmc_client_get_cached_or_fetch_with_cache_miss_and_metrics(self):
        """Test BMCAMIDevXClient.get_cached_or_fetch with cache miss and metrics."""
        cache = main.IntelligentCache()
        metrics = main.Metrics()
        client = main.BMCAMIDevXClient(httpx.AsyncClient(), cache, metrics, main.ErrorHandler(main.Settings(), metrics))
    ...
    def test_bmc_client_get_cached_or_fetch_with_no_cache(self):
        """Test BMCAMIDevXClient.get_cached_or_fetch with no cache."""
        metrics = main.Metrics()
        client = main.BMCAMIDevXClient(httpx.AsyncClient(), None, metrics, main.ErrorHandler(main.Settings(), metrics))
    ...
    def test_bmc_client_get_cached_or_fetch_with_caching_disabled(self):
        """Test BMCAMIDevXClient.get_cached_or_fetch with caching disabled."""
        settings = main.Settings(enable_caching=False)
        cache = main.IntelligentCache()
        metrics = main.Metrics()
        client = main.BMCAMIDevXClient(httpx.AsyncClient(), cache, metrics, main.ErrorHandler(settings, metrics))
    ...
    def test_get_assignments_core_with_all_parameters(self):
        """Test _get_assignments_core with all parameters."""
        with patch.object(main.bmc_client, 'get_cached_or_fetch', return_value=asyncio.coroutine(lambda: {"assignments": []})()):
            result = asyncio.run(main._get_assignments_core("TEST-SRID", "DEV", "ASSIGN-001"))
            assert result == {"assignments": []}

    def test_create_assignment_core_with_all_parameters(self):
        """Test _create_assignment_core with all parameters."""
        assignment_data = {
            "srid": "TEST-SRID",
        ...
        with patch.object(main.bmc_client, 'make_request', return_value=asyncio.coroutine(lambda: assignment_data)()):
            result = asyncio.run(main._create_assignment_core(assignment_data))
            assert result == assignment_data

    def test_get_assignment_details_core_with_all_parameters(self):
        """Test _get_assignment_details_core with all parameters."""
        with patch.object(main.bmc_client, 'get_cached_or_fetch', return_value=asyncio.coroutine(lambda: {"assignment": "details"})()):
            result = asyncio.run(main._get_assignment_details_core("TEST-SRID", "ASSIGN-001"))
            assert result == {"assignment": "details"}

    def test_get_assignment_tasks_core_with_all_parameters(self):
        """Test _get_assignment_tasks_core with all parameters."""
        with patch.object(main.bmc_client, 'get_cached_or_fetch', return_value=asyncio.coroutine(lambda: {"tasks": []})()):
            result = asyncio.run(main._get_assignment_tasks_core("TEST-SRID", "ASSIGN-001"))
            assert result == {"tasks": []}
...
class TestOpenAPIServer100PercentCoverage:
    """Tests to achieve 100% coverage for openapi_server.py"""
    ...
    def test_simple_cache_get_oldest_entry_age_with_entries(self):
        ...
        entry1 = openapi_server.SimpleCacheEntry("data1", datetime.now() - timedelta(seconds=100))
        entry2 = openapi_server.SimpleCacheEntry("data2", datetime.now() - timedelta(seconds=50))
    ...
    def test_get_server_health_api_exception(self):
        ...
        with patch.object(openapi_server.rate_limiter, 'acquire', return_value=True):
            with patch.object(openapi_server.http_client, 'get', side_effect=Exception("API error")):
                ...
                assert data['bmc_api_status'] == "error"
    ...
    def test_get_error_recovery_status(self):
        ...
        assert 'retryable_errors' in data
        assert 'non_retryable_errors' in data
    ...
    def test_create_assignment_interactive_success(self):
        ...
        with patch.object(openapi_server.rate_limiter, 'acquire', return_value=True):
            with patch.object(openapi_server.http_client, 'post', return_value=Mock(status_code=201, json=lambda: {"assignmentId": "TEST-001"})):
                result = asyncio.run(openapi_server.create_assignment_interactive.fn(mock_ctx))
                assert "Assignment created successfully" in result
    ...
    def test_create_assignment_interactive_exception(self):
        ...
        with patch.object(openapi_server.rate_limiter, 'acquire', return_value=True):
            with patch.object(openapi_server.http_client, 'post', side_effect=Exception("API error")):
                result = asyncio.run(openapi_server.create_assignment_interactive.fn(mock_ctx))
                assert "Error creating assignment" in result
