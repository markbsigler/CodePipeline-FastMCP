...
class TestOpenAPIServerCoverage:
    ...
    def setup_method(self):
        ...
        self.temp_spec = tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False)
        self.temp_spec.write(json.dumps({
            "openapi": "3.0.0",
        ...
        with patch.object(openapi_server, 'openapi_spec_path', Path(self.temp_spec.name)):
    ...
    def test_simple_rate_limiter_acquire_no_tokens(self):
        ...
        rate_limiter = openapi_server.SimpleRateLimiter(requests_per_minute=60, burst_size=1)
    ...
    def test_simple_metrics_to_dict_with_cache_stats(self):
        ...
        with patch.dict('openapi_server.__dict__', {'cache': Mock(get_stats=Mock(return_value={'size': 10}))}):
    ...
    def test_simple_error_handler_categorize_http_status_error(self):
        ...
        status_error = openapi_server.httpx.HTTPStatusError("Server error", request=Mock(), response=mock_response)
    ...
    def test_simple_error_handler_categorize_authentication_error(self):
        ...
        status_error = openapi_server.httpx.HTTPStatusError("Unauthorized", request=Mock(), response=mock_response)
        result = error_handler.categorize_error(status_error, "test_operation")
    ...
    def test_simple_error_handler_categorize_not_found_error(self):
        ...
        status_error = openapi_server.httpx.HTTPStatusError("Not found", request=Mock(), response=mock_response)
        result = error_handler.categorize_error(status_error, "test_operation")
    ...
    def test_simple_error_handler_categorize_rate_limit_error(self):
        ...
        status_error = openapi_server.httpx.HTTPStatusError("Rate limited", request=Mock(), response=mock_response)
        result = error_handler.categorize_error(status_error, "test_operation")
    ...
    def test_simple_error_handler_should_retry_http_retryable_status(self):
        ...
        status_error = openapi_server.httpx.HTTPStatusError("Service unavailable", request=Mock(), response=mock_response)
    ...
    def test_simple_error_handler_should_retry_http_non_retryable_status(self):
        ...
        status_error = openapi_server.httpx.HTTPStatusError("Bad request", request=Mock(), response=mock_response)
    ...
    def test_get_server_metrics(self):
        ...
        # Update metrics
        openapi_server.metrics.total_requests = 10
        openapi_server.metrics.successful_requests = 8
        openapi_server.metrics.failed_requests = 2
    ...
    def test_get_rate_limiter_status(self):
        ...
        assert data['configuration']['requests_per_minute'] == openapi_server.rate_limiter.requests_per_minute
    ...
    def test_cleanup_expired_cache(self):
        ...
        # Add expired entry
        expired_key = openapi_server.cache._generate_key("expired_method", param="value")
    ...
    def test_create_assignment_interactive_success(self):
        ...
        with patch.object(openapi_server.rate_limiter, 'acquire', return_value=True):
            with patch.object(openapi_server.http_client, 'post', return_value=mock_response):
                result = asyncio.run(openapi_server.create_assignment_interactive.fn(mock_ctx))
                data = json.loads(result)
    ...
    def test_health_check_route_success(self):
        ...
        with patch.object(openapi_server, 'get_server_health', new_callable=AsyncMock, return_value='{"status": "healthy"}') as mock_health:
    ...
    def test_health_check_route_exception(self):
        ...
        with patch.object(openapi_server, 'get_server_health', side_effect=Exception("Health check error")):
            response = asyncio.run(openapi_server.health_check_route(mock_request))
    ...
    def test_metrics_route_exception(self):
        ...
        with patch.object(openapi_server, 'get_server_metrics', side_effect=Exception("Metrics error")):
            response = asyncio.run(openapi_server.metrics_route(mock_request))
    ...
    def test_get_assignment_resource_cache_hit(self):
        ...
        # Pre-populate cache
        asyncio.run(openapi_server.cache.set("get_assignment", {"assignmentId": "TEST-001"}, srid="TEST"))
